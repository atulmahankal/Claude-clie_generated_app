...
class AuthService {
  ...
  constructor() {
    this.httpPort = parseInt(process.env.HTTP_PORT || '3001');
    this.grpcPort = parseInt(process.env.GRPC_PORT || '50051');
  ...
  async start(): Promise<void> {
    try {
      this.baseApp.logger.info('Starting auth service...');
      ...
      this.baseApp.logger.info('Auth service started successfully', {
        httpPort: this.httpPort,
        grpcPort: this.grpcPort,
      });
    } catch (error) {
      this.baseApp.logger.error('Failed to start auth service', error);
      process.exit(1);
    }
  }
  ...
  async stop(): Promise<void> {
    this.baseApp.logger.info('Stopping auth service...');

    if (this.grpcServer) {
      await this.grpcServer.stop();
    }

    // Close HTTP server (would need to store server instance)
    // For now, just log

    this.baseApp.logger.info('Auth service stopped');
  }
}
